...
for ((i = 1; i <= $rec_column3; i += 1)); do
     ...
     ## Currentlly this timeout is defaulted to 30 minutes (108000 seconds), which is about ten mintues over the expected runtime just incase a simulation is running slowly.
     ...
     (sleep 108000 && kill -HUP $pid) 2>/dev/null & watcher=$!
